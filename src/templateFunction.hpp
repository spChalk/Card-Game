...
void pushNtimes(std::shared_ptr < std::queue< std::shared_ptr < D > > > deck , std::unordered_map<std::string , std::vector<uint16_t> >::iterator j ,const E type  , uint16_t total , uint16_t * times, const std::shared_ptr < std::vector< std::string >> cardTexts ) {

...
void pushNtimes(std::shared_ptr < std::queue< std::shared_ptr < D > > > deck , std::unordered_map<std::string , std::vector<uint16_t> >::iterator j ,const E type  , uint16_t total , uint16_t * times, const std::string cardText = "Random text") {

  uint16_t temp = total;
  while (temp--)
  {
    if (j->second.size() == 6) {  // Follower
      deck->push (std::dynamic_pointer_cast<D>( std::make_shared< Follower >(j->first , j->second[0] , j->second[1] , j->second[2] , j->second[3] , cardText , j->second[4] , j->second[5] , (const enum FollowerType)type , total) ) );
    }
    else if (j->second.size() == 7) {  // Item
      deck->push (std::dynamic_pointer_cast<D>(std::make_shared< Item >(j->second[6] , j->first , j->second[0] , j->second[1] , j->second[2] , j->second[3] , cardText , j->second[4] , j->second[5] , (const enum ItemType)type , total)));
    }
